...
if __name__ == '__main__':
    ...
    exp_inds, feat_means = zip(*map(get_df_quantiles, experiments.iterrows()))
    #%%    
    ...
    #%%
    avg_dat = feat_means_df.groupby('strain').agg('mean')
    #%%
    avg_dat.to_csv('CeNDR_all_avg.csv')
    #%%
    CV = (avg_dat.max() - avg_dat.min())/avg_dat.abs().mean()
    avg_dat[CV.sort_values()[-20:].index[::-1]].to_csv('CeNDR_test_avg.csv')
    #%%
#    #%%
#    strain_test = 'CB4856'
